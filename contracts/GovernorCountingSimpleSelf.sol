...
abstract contract GovernorCountingSimpleSelf is Governor {
    ...
    //  */
    function register_hacker(string memory _name, uint256 _hackathonId)
        public
        returns (uint256)
    {
     ...
     */
    function hasHackerRegistedForHackathon(uint256 _hackathonId)
        public
        view
        returns (bool)
    {
        ...
        uint256 numOfHackers = hackers.length;
        for (uint256 i = 0; i < numOfHackers; i++) {
            if (hackers[i].hackerAdd == msg.sender) {
                result = true;
                break;
            }
    ...
    function add_submission(
        ...
        require(
            hasHackerRegistedForHackathon(_hackathonId),
            "Hacker NOT registered! Please register yourself"
        );
        Hacker[] storage hackers = hackathonIdToHackathon[_hackathonId].hackers;
        for (uint256 i = 0; i < hackers.length; i++) {
        ...
        returns (string memory)
    {
        ...
        Hackathon memory hackathon = hackathonIdToHackathon[
            ProposalIdToHackathonId[_proposalId]
        ];
    ...
    {
        return
            hackathonIdToHackathon[ProposalIdToHackathonId[_proposalId]]
                .blockNum;
    }
    ...
    function _countVote(
        ...
        require(
            ...
            "GovernorVotingSimpleSelf: vote already cast"
        );
